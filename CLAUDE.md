...
- **Reverse flow reverses geometry, never signs.** A component on an edge with negative mdot is evaluated against a *reversed shadow* of itself (profile flipped, `Di_US`/`Di_DS` swapped). Do **not** patch new asymmetric components with `sign(Q)*friction(|Q|)` shortcuts — extend `_reversed_component()` in [network.py](network.py) instead. Check valves are not resistance-asymmetric: they seal perfectly, and both network solvers pin a CV edge to exactly zero reverse flow via a complementarity residual. See `network.md` § "Reverse-flow handling".
