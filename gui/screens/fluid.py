...
class FluidScreen(QWidget):
    ...
    def _build_incompressible_panel(self):
        self.i_rb_forward = QRadioButton("Solve for pressure profile (given flow rate)")
        self.i_rb_inverse = QRadioButton("Solve for flow rate (given outlet pressure)")
        self.i_rb_forward.setChecked(True)
        i_mode_group = QButtonGroup(self)
        i_mode_group.addButton(self.i_rb_forward)
        i_mode_group.addButton(self.i_rb_inverse)
        self.i_rb_forward.toggled.connect(self._apply_incompressible_mode)
        self.i_rb_inverse.toggled.connect(self._apply_incompressible_mode)

        i_mode_box = QGroupBox("Operating mode")
        i_mode_v = QVBoxLayout(i_mode_box)
        i_mode_v.addWidget(self.i_rb_forward)
        i_mode_v.addWidget(self.i_rb_inverse)
        ...
        self.i_p_inlet   = _LabeledField("0",    U.PRESSURE,  "psi")
        self.i_p_outlet  = _LabeledField("0",    U.PRESSURE,  "psi")
        ...
        other.addRow("Inlet pressure:",   self.i_p_inlet.widget())
        other.addRow("Outlet pressure:",  self.i_p_outlet.widget())
        other.addRow("Flow rate:",        self.i_flow.widget())

        panel = QWidget()
        v = QVBoxLayout(panel)
        v.addWidget(i_mode_box)
        v.addWidget(rho_box)
        v.addLayout(other)
        v.addStretch()

        self._apply_incompressible_mode()
        return panel

    def _apply_incompressible_mode(self):
        """Enable / disable inverse-only and forward-only fields.

        Disabled fields keep their text so toggling back doesn't lose input.
        The flow-rate value entry is disabled in inverse mode but its unit
        dropdown stays enabled -- it becomes the display unit for the
        solved mdot shown on the Results summary.
        """
        inverse = self.i_rb_inverse.isChecked()
        self.i_p_outlet.edit.setEnabled(inverse)
        self.i_p_outlet.combo.setEnabled(inverse)
        self.i_flow.edit.setEnabled(not inverse)
    ...
    def _build_compressible_panel(self):
        ...
        self.c_p_outlet = _LabeledField("800",  U.PRESSURE,    "psi")
        ...
        self.c_rb_forward = QRadioButton("Solve for pressure profile (given flow rate)")
        self.c_rb_inverse = QRadioButton("Solve for flow rate (given outlet pressure)")
        self.c_rb_forward.setChecked(True)
        c_op_group = QButtonGroup(self)
        c_op_group.addButton(self.c_rb_forward)
        c_op_group.addButton(self.c_rb_inverse)
        self.c_rb_forward.toggled.connect(self._apply_compressible_mode)
        self.c_rb_inverse.toggled.connect(self._apply_compressible_mode)

        c_op_box = QGroupBox("Operating mode")
        c_op_v = QVBoxLayout(c_op_box)
        c_op_v.addWidget(self.c_rb_forward)
        c_op_v.addWidget(self.c_rb_inverse)
        ...
        cond.addRow("Outlet pressure:",   self.c_p_outlet.widget())
        ...
        panel = QWidget()
        v = QVBoxLayout(panel)
        v.addWidget(c_op_box)
        v.addWidget(comp_box, 1)
        v.addLayout(cond)
        v.addWidget(mode_box)

        self._apply_compressible_mode()
        return panel

    def _apply_compressible_mode(self):
        """Enable / disable inverse-only and forward-only compressible fields.

        Mirror of _apply_incompressible_mode -- toggles the P_outlet entry
        and disables the flow-rate value entry (unit stays selectable for
        the solved-mdot display).
        """
        inverse = self.c_rb_inverse.isChecked()
        self.c_p_outlet.edit.setEnabled(inverse)
        self.c_p_outlet.combo.setEnabled(inverse)
        self.c_flow.edit.setEnabled(not inverse)
    ...
    def _finalize_incompressible(self):
        ...
        P_in = self.i_p_inlet.quantity()
        if P_in is None:
            raise ValueError("Inlet pressure is required.")
        P_inlet_Pa = P_in.to("Pa").magnitude

        inverse = self.i_rb_inverse.isChecked()
        if inverse:
            P_out = self.i_p_outlet.quantity()
            if P_out is None:
                raise ValueError("Outlet pressure is required for the inverse solve.")
            P_outlet_Pa = P_out.to("Pa").magnitude
            ...
            self.state.flow_rate_display_unit = self.i_flow.combo.currentText()
            self.state.P_outlet_Pa            = P_outlet_Pa
            self.state.solve_mode             = "inverse"
        else:
            flow = self.i_flow.quantity()
            if flow is None:
                raise ValueError("Flow rate is required.")
            self.state.flow_rate              = flow
            self.state.flow_rate_display_unit = None
            self.state.P_outlet_Pa            = None
            self.state.solve_mode             = "forward"

        self.state.fluid           = fluid
        self.state.P_inlet_Pa      = P_inlet_Pa
        self.state.T_inlet_K       = None
        self.state.solved_mdot_kgs = None
    ...
    def _finalize_compressible(self):
        ...
        if P is None or T is None:
            raise ValueError("Inlet pressure and temperature are required.")
        ...
        inverse = self.c_rb_inverse.isChecked()
        if inverse:
            P_out = self.c_p_outlet.quantity()
            if P_out is None:
                raise ValueError("Outlet pressure is required for the inverse solve.")
            P_outlet_Pa = P_out.to("Pa").magnitude
            if P_outlet_Pa >= P_Pa:
            ...
            self.state.flow_rate              = None
            self.state.flow_rate_display_unit = self.c_flow.combo.currentText()
            self.state.P_outlet_Pa            = P_outlet_Pa
            self.state.solve_mode             = "inverse"
        else:
