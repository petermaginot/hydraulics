...
Same `dP(fluid, flow_rate, P_inlet=None)` and `dmdot(fluid, P_inlet, P_outlet)` shape as `Valve`, with the same optional `F_L` cavitation gate. Both methods model forward passage only: reverse flow is blocked exactly by the network solvers, which treat a check valve as a perfect seal and pin its edge to zero flow via a complementarity residual (see [network.md](network.md) § "Reverse-flow handling").
...
- `energy_tol`, `dPdL_rel_tol`, `Ma_change_tol`, `max_split_depth` — adaptive bisection tolerances forwarded into each slice: the per-slice stagnation-enthalpy residual (on the corrected state), the relative dP/dL change and the Mach-number change across the slice (both on the trial state), and the recursion-depth cap.
- `correction_skip_rel_tol` — skips the Heun corrected-state flash when the correction is negligible relative to `(P_in, T_in)` (fires on near-stagnant slices, e.g. while the network LM solver probes tiny trial mdots), so those slices stay at a single flash.
...
- **`compressible_pipe_segment(fs, dL, dz, D_h, roughness, q_wall=0.0, isothermal=False, mu=None, ...)`** — This one is the real beast!. It solves coupled dP/dL and dT/dL ODEs (or the simpler isothermal dP/dL equation) over a single pipe slice using a forward-Euler step with a one-iteration energy-balance correction.
  ...
  After the trial step, the slice is corrected with the Heun trapezoidal predictor-corrector (dP/dL averaged inlet-to-trial-outlet) plus a one-step Newton projection of `T_out` onto stagnation-enthalpy conservation, and three convergence metrics decide whether the slice must be bisected:
  1. The relative change in dP/dL between inlet and trial-outlet fluid properties vs `dPdL_rel_tol`, and the Mach-number change across the slice vs `Ma_change_tol` — both gauge whether the inlet-property linearization is valid over the slice, so they are judged on the *trial* state.
  2. If not isothermal, an energy balance (stagnation enthalpy + heat in − stagnation enthalpy out) vs `energy_tol`. This is judged on the **corrected** state — what the function actually returns — not the Euler predictor: the corrector conserves stagnation enthalpy to O(dL⁴) versus the predictor's O(dL²), so gating on the predictor would force needless splits on strongly-accelerating (high-Mach) slices the corrector has already resolved. The corrector is computed before the split decision (only when the cheap dP/dL and Mach gates pass), so no EOS flash is wasted on a slice that splits anyway.

  If any metric is out of tolerance, the slice is recursively bisected (halving `dL`, `dz`, and `q_wall`) until convergence or `_max_split_depth` is exhausted; the depth-exceeded message reports `corrected_energy_error`. Sonic / two-phase / unphysical-EOS-update conditions raise `RuntimeError`.
...
- **`compressible_changing_area(fs, A_out)`** — ideal gas isentropic outlet `(P, T)` for an area change, using the area-Mach relation to find the subsonic outlet Mach number and recovering static conditions from total-condition ratios. `gamma = Cp/Cv` is taken from `fs.AS` at inlet. Returns `(P_out, T_out)` without mutating the FlowState. This function is used to compute the initial guess for the compressible_changing_area_K function. **Large-expansion short-circuit:** the brentq bracket for the subsonic Mach root is fixed at `Ma_lo = 1e-9`, which only reaches `A/A* ≲ 5.8e8` (air); a very large expansion — or any area change evaluated at a vanishingly small `mdot` (the sonic area `A* ∝ mdot`, so an inverse solver's tiny trial flow can push `A/A*` past `1e13`) — puts the deeply-subsonic root below `Ma_lo`. Rather than mis-fire as "could not bracket a subsonic root," the function detects that regime via the small-Mach asymptote `Ma_out ≈ (2/(γ+1))^((γ+1)/(2(γ-1))) / (A/A*)` and, when that estimate is below `1e-6`, returns stagnation-based static conditions directly (`P_out ≈ P_total`, `T_out ≈ T_total`) — the physically correct limit, since a huge expansion just decelerates the flow to rest. A choking *contraction* gives `A/A* < 1` (small, not large), so it skips the short-circuit and still raises.
